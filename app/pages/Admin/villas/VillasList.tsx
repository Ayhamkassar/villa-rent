...
export default function VillasList() {
  ...
  const fetchFarms = useCallback(async () => {
    try {
      ...
      // البحث مباشر على client-side
      const filtered = res.data.filter((farm: any) =>
  ...
  const confirmAndDelete = async (farmId: any) => {
      ...
      { text: 'إلغاء', style: 'cancel' },
      {
        text: 'حذف', style: 'destructive', onPress: async () => {
          try {
            ...
            setFarms((prev) => prev.filter((f: any) => f._id !== farmId));
  ...
  const renderFarm = ({ item }: any) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => router.push({ pathname: "/pages/Admin/villas/VillaDetails", params: { id: item._id } })}
    >
      <Image
        source={{
          uri:
  ...
  return (
    <LinearGradient colors={['#74EBD5', '#ACB6E5']} style={{ flex: 1 }}>
      <View style={styles.overlay}>
        {/* SEARCH */}
        <TextInput
          placeholder="ابحث عن المزارع أو الفلل..."
          value={search}
